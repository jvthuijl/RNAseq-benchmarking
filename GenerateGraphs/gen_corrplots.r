...
calcAbundance <- function(dir, nrows = 20){
  ...
  for (i in dir(dir)) {
    ...
    outputAbundance[[i]] <- tmpAbundance$est_counts
  }
    setwd(prevWd)
...
samplegroups = paste("illumina_probe", rep(coverage, each=3), kmers, sep="/")
sampleprefixes = paste("illumina_probe", rep(coverage, each=3), kmers, sep="_")
...
for (group in samplegroups){
    abundance = calcAbundance(paste(wd, "base", group, sep = "/"), Inf)
    abundance$sd = as.numeric(abundance$sd)
    abundance = na.omit(abundance)
    qplot(sd, data=abundance, geom = "histogram", binwidth = 1)
    filename <- gsub("/", "-", group)
    write.table(abundance[abundance$sd > 40,paste0(filename, ".txt")], col.names=TRUE, sep=",")
   	ggsave(paste0(filename, ".pdf"), device = "pdf", width = 10, height = 10)
}
dev.off()
...
#     first = FALSE
#     print(head(abundance))
#     merged <- data.frame(row.names = rownames(abundance))
#     #colnames(final) <- "target_id"
#   }
#   # for each sample add a column
#   merged <- data.frame(merged, new = abundance$mean)
#   names(merged)[names(merged) == 'new'] <- paste0("mean.", gsub("/", "_", group))
# }
